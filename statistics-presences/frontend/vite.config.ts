...
export default defineConfig(({ mode }) => ({
  // Servi sous /statistics-presences par entcore (cf. view massmailing-react.html -> /statistics-presences/public/index.js)
  base: mode === 'production' ? '/statistics-presences' : '',
  resolve: {
    dedupe: [
      'react',
      'react-dom',
  ...
  server: {
    port: 4207,
    proxy: {
      '/statistics-presences': proxyTarget,
